...
namespace mcb
{
    ...
    using leda::d_int_set;
      ...
      template < class W, class prio_impl = leda::bin_heap > class sgraph {
      public:
	...
	sgraph( const graph & G, const edge_num & _enumb )
	...
	{
	    return sg.is_hidden( epos[e] ) || sg.is_hidden( eneg[e] );
	}
	...
	bool check_valid( node u, node_array < edge > &pred, d_int_set & V )
	{
		...
		if ( Stemp.member( enumb( remap[pred[t]] ) ) ) {
		    ...
		    Stemp.insert( enumb( remap[pred[t]] ) );
	...
	void init_signed_graph(  )
	{
	    ...
	    forall_edges( e, g ) {
		edge e1 =
		    sg.new_edge( pos[g.source( e )], pos[g.target( e )] );
		epos[e] = e1;
		edge e2 =
		    sg.new_edge( neg[g.source( e )], neg[g.target( e )] );
		eneg[e] = e2;
		remap[e1] = e;
		remap[e2] = e;
		zero_length_edges = false;
