...
namespace mcb
{
    ...
    void MCB_output_basis( const graph & g,
			   ...
			   const edge_num & enumb, array < d_int_set > &C )
    {
	edge e;
    ...
    bool MCB_verify_basis( const graph & g,
			   array < d_int_set > &mcb,
			   const edge_num & enumb, bool check_cycle_basis )
    {
	edge e;
	node v;
...
#ifdef LEP_STATS
	std::cout << "LEP_STATS: cycle computation time: " << Tcycle << std::
	    endl;
	std::
	    cout << "LEP_STATS: orthogonal base maintain time: " << Torthog <<
	    std::endl;
#endif
    ...
    // lists version
    int MIN_CYCLE_BASIS_DEPINA( const graph & g, array < list < edge > >&mcb )
    {
    ...
    void MCB_construct_superset( const graph & G,
				 sortseq < two_tuple < int, int >,
				 d_int_set > &super, const edge_num & enumb )
    {
	...
	std::cout << "LEP_STATS: horton set size: " << super.
	    size(  ) << std::endl;
#endif
...
#ifdef LEP_STATS
	std::
	    cout << "LEP_STATS: horton set queries: " << lep_stats_queries <<
	    std::endl;
#endif
    ...
    int MIN_CYCLE_BASIS_HYBRID( const graph & g, array < list < edge > >&mcb )
